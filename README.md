...
각 카드의 **선택지 추가**는 기본 해결 선택지를 추가합니다. **다음 단계 추가**는 새 안내 카드와 동일합니다 연결을 함께 만듭니다. 추가 후 문구를 편집하고 선택지를 클릭해 이동 단계 또는 종료 결과를 지정하세요. **최근 추가 취소**는 현재 페이지에서 최근 추가 직전 상태로 되돌립니다(최대 20회). 이후 수정한 내용도 해당 시점으로 돌아가므로 사용에 유의하세요. 새로고침하면 취소 이력은 초기화됩니다. 최종 반영은 **저장하고 적용**을 눌러야 합니다.
