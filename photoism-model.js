...
export function mount(host){
 ...
 let openness=0,frame=0,disposed=false,tween=null,model=null,door=null,pending=null;
 ...
 function draw(now=performance.now()){frame=0;if(disposed)return;let done=null;if(tween){const t=reduced?1:Math.min(1,(now-tween.start)/1050),ease=t<.5?4*t*t*t:1-Math.pow(-2*t+2,3)/2;openness=tween.from+(tween.to-tween.from)*ease;if(t===1){done=tween.done;tween=null;}}
  const distance=Math.max(3.65,host.clientHeight/Math.max(host.clientWidth,1)*3.12);if(door)door.rotation.y=openness*Math.PI*110/180;camera.position.set(0,1.16,distance);camera.lookAt(target);host.dataset.doorState=tween?'moving':openness>.99?'open':'closed';renderer.render(scene,camera);host.dispatchEvent(new Event('modelrender'));if(tween)frame=requestAnimationFrame(draw);if(done)done();
 }
 ...
 new GLTFLoader().load(new URL('./assets/photoism-kiosk-pointing.glb',document.baseURI).href,gltf=>{if(disposed){release(gltf.scene);return;}model=gltf.scene;door=model.getObjectByName('kiosk_front_door');if(!door){release(model);model=null;host.dispatchEvent(new CustomEvent('modelerror'));return;}scene.add(model);loading.remove();host.dataset.modelSource='blender';host.dataset.modelState='ready';if(pending){const next=pending;pending=null;view(next.mode,next.done);}else request();},undefined,()=>{if(!disposed){host.dataset.modelState='error';host.dispatchEvent(new CustomEvent('modelerror'));}});
